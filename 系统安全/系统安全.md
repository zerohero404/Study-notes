...
- 修改数据包的目标地址
- DNAT 网络拓扑<br>
<img width="613" height="305" alt="系统安全-07" src="https://github.com/user-attachments/assets/1ba665dc-db59-478c-ad88-e88d4015a13e" /><br>
- 前提条件
  - 局域网的 Web 服务器能够访问 Internet
  - 网关的外网 IP 地址有正确的DNS 解析记录
  - Linux 网关支持 IP 路由转发
- 步骤演示
  - 需要两块网卡，eth0 和 eth1，eth0 为内网网卡（192.168.1.2），eth1 为外网网卡（218.29.30.31）
  - web 服务器要把网关设置为网关服务器的 IP 地址
  - 网关服务器需要开启路由转发
  - vim /etc/sysctl.conf
    - 将 net.ipv4.ip_forward = 0 修改为 net.ipv4.ip_forward = 1 # 开启路由转发
  - sysctl -p # 刷新配置文件
  - iptables -F # 清空防火墙规则
  - iptables -t nat -A PREROUTING -i eth1（外网网卡） -d 218.29.30.31（本机的公网地址） -p tcp --dport 80 -j DNAT --to-destination 192.168.1.6（web 服务器 IP）
  - service iptables save # 保存防火墙配置

#### 5.4.3 常见 Iptables 相关命令（补充）
- 导出（备份）规则：iptables-save 工具，可结合重定向输出保存到指定文件
  - iptables-save > backup.iptables # 将当前防火墙规则备份成 backup.iptables 文件
- 导入（还原）规则：iptables-restore 工具，可结合重定向输入指定规则来源
  - iptables-restore < backup.iptables # 将防火墙规则还原为 backup.iptables 内的规则
- 脚本位置：/etc/init.d/iptables
- 规则文件位置：/etc/sysconfig/iptables
- 防火墙规则只在计算机处于开启状态时才有效。如果系统被重新引导，这些规则就会自动被清除并重设。要保存规则以便今，请使用以下命令进行载入
  - sbin/service iptables save
- 保存后的规则文件保存在 /etc/sysconfig/iptables，文件中的规则会在服务启动或重新启动时（包括机器被重新引导时） 被应用

### 5.5 Centos7 使用 Iptables 为 netfilter 应用层工具
...
function support_distro(){
        if [ -z "`egrep -i "centos" /etc/issue`" ];then
                echo "Sorry,iptables script only support centos system now." exit 1
        fi
        }
...
# 允许SSH 端口的连接,脚本自动侦测目前的SSH 端口,否则默认为 22 端口
...
# 允许 DNS
[ ! -z "$nameserver1" ] && $IPT -A OUTPUT -p udp -m udp -d $nameserver1 --dport 53 -j ACCEPT [ ! -z "$nameserver2" ] && $IPT -A OUTPUT -p udp -m udp -d $nameserver2 --dport 53 -j ACCEPT

# 保存规则并重启 IPTABLES service iptables save
