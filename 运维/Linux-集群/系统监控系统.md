...
- 配置 windos 端监控
&ensp;&ensp;被监控端安装 NSClient++-0.3.8-Win32.msi
&ensp;&ensp;安装完成后修改配置文件 NSC.ini 把需要的库都打开
&ensp;&ensp;在监控服务器上修改 nagios 配置文件 nagios.cfg
```bash
define host{
...
- 配置 linux 端监控
&ensp;&ensp;useradd nagios # 创建用户
&ensp;&ensp;tar -zxf nagios-plugins-1.4.15.tar.gz # 为了安装 nrpe，先安装 nagios-plugins-1.4.15.tar.gz 插件
&ensp;&ensp;cd nagios-plugins-1.4.15
```bash
...
&ensp;&ensp;tar -zxvf nrpe-2.12.tar.gz # 安装 nrpe
&ensp;&ensp;cd nrpe-2.12.tar.gz
...
&ensp;&ensp;vim /usr/local/nagios/etc/nrpe.cfg # 配置 nrpe 信息

```bash
 allowed_hosts=192.168.216.251,127.0.0.1
```

&ensp;&ensp;/usr/local/nagios/bin/nrpe -c /usr/local/nagios/etc/nrpe.cfg –d
&ensp;&ensp;cd nagios-nrpe_2.8.1 # 服务端安装 nrpe 插件
...
&ensp;&ensp;vi /usr/local/nagios/etc/objects/commands.cfg # commands.cfg 定义外部构件 nrpe
